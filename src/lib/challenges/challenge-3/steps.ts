import { findAllButtonComponents, findAllIconComponents } from '@/lib/challenges/codeFinders';
...
const containsAccessibilityState = (code: string) => {
  ...
  return !!minusButton?.match(/accessibilityState={\s*{\s*disabled:\s*quantity\s*===\s*0\s*}\s*}/gm);
};
...
const touchableOpacityIs48By48 = (code: string) => {
  ...
  const touchableOpacityStyleHas24Padding = touchableOpacityStyle.match(/padding:\s*12/)?.length === 1;
  const touchableOpacityStyleHasNegative24Margin = touchableOpacityStyle.match(/margin:\s-12/)?.length === 1;

  return touchableOpacityStyleHas24Padding && touchableOpacityStyleHasNegative24Margin;
};
...
export const allIconsAre24By24Step: ChallengeStep = {
  ...
  failureMessage: 'Not all icons are 24x24 pixels! The design team insists the icon size remains unchanged.',
  test: allIconsAre24By24
};

export const allButtonsAre48By48Step: ChallengeStep = {
  successMessage: 'All buttons are 48x48 pixels',
  failureMessage: 'Not all buttons are 48x48 pixels, users may struggle to tap the right area to activate the button',
